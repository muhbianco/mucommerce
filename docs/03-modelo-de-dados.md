...
> - Etapa D (migrations `0012`–`0016`) no lugar de `product_tags(tag)`, `product_options*`, `product_modifier*`, `events` e `event_products` acima:
>   - **`products.status`** ganha `paused` (publicado, fora de venda). `paused_at`, `paused_reason` e `paused_by_actor` ficam em `products` e em `product_variants`, e a variante também pode ser `paused`. `PUBLISHED_STATUSES = {active, paused}` é o que aparece na vitrine; só `active` vende.
>   - **`tags`**: `slug` ≤80, `name` ≤60, UNIQUE `(tenant_id, slug)`. **`product_tags`**: UNIQUE `(tenant_id, product_id, tag_id)`, FKs compostas e índice `(tenant_id, tag_id, product_id)` para o filtro da vitrine.
>   - **`products.options`** é JSON `[{name, values}]` (a matriz é o produto cartesiano). Cada variante carrega `option_values`, e as que saem da matriz são arquivadas, nunca apagadas.
>   - **`products.modifier_groups`** é JSON `[{id, name, min_select, max_select, modifiers:[{id, name, price_cents, active}]}]`, com ids estáveis (carrinho e linhas de pedido vão referenciar). Fica em JSON porque ninguém consulta por modificador.
>   - **`events`**: 1:1 com um produto `kind=ticket`, UNIQUE `(tenant_id, product_id)`. Campos `starts_at`, `ends_at`, `venue_*`, `city`, `online_url` (privado), `capacity`, `status ∈ scheduled|postponed|cancelled` e `status_note`; índice `(tenant_id, starts_at, id)`. Nome, descrição, imagens e publicação ficam no produto, e as imagens usam o dono `product` (não há dono `event`).
>   - **`event_lots`**: `event_id`, `variant_id` (UNIQUE: um lote por variante), `quantity`, `sales_starts_at`, `sales_ends_at` e `position`. O preço é o da variante; os ingressos são o estoque dela.
...
> - Estoque (migration `0006_inventory`): quantidades em **inteiros de milésimos** da unidade (`on_hand_milli`, `reserved_milli`, `min_level_milli`, `qty_milli`, `balance_after_milli`, todos BIGINT), não `DECIMAL`, para ter soma exata em qualquer banco (o SQLite dos testes não tem decimal fixo). Custo em `unit_cost_micro` (micro-reais). `inventory_balances`/`inventory_movements` usam `variant_id` com FK composta em vez de `item_type/item_id`; insumos (F5) entram como coluna própria. O saldo nasce junto com a variante. `stock_adjustments` é o cabeçalho de cada operação do painel (`receipt|loss|adjustment|count`); o movimento aponta para ele (`reference_type=stock_adjustment`).
