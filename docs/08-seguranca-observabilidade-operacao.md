...
- **Repos/CI**: monorepo `mucommerce` com GitHub Actions: `lint` (ruff, mypy, eslint, tsc), `test-api` (pytest unit SQLite + integração MariaDB 10.11 service + suíte tenancy), `test-web` (vitest + Playwright smoke com `FakeProvider`), `build` (Docker buildx, tags `sha-<short>` e `vX.Y.Z`, push Docker Hub `muhrilobianco/commerce_api|commerce_web`), `deploy-staging` (Portainer webhook/StackUpdate) automático em `main`, `deploy-prod` manual por tag (environment protection).
