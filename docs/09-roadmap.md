...
Sem prazos; estimativas relativas estão no [backlog](10-backlog.md). Cada fase termina com deploy em staging, critérios de aceite verificados e runbook de rollback.

## Fase 0 — Fundação

**Escopo**: monorepo `mucommerce` (`apps/api-commerce`, `apps/web`, `infra`, `docs`, `.github`), esqueleto FastAPI com convenções da `api-agents` + tenancy, Alembic `0001–0002`, Celery + outbox + `processed_events`, `idempotency_keys`, `audit_log`, CI (lint, testes, build, push), stack Swarm `commerce` + `commerce-staging`, migrate one-shot, bootstrap MariaDB (usuários, bind restrito), MinIO buckets/policy/service account, Traefik `providers.http`, Next.js esqueleto com middleware de Host, observabilidade base (logs JSON, `/healthz`, `/readyz`, Sentry, métricas), fork Chatwoot migrado (`mb/main`), Redis DBs reservados.

**Aceite**: `GET https://staging.loja.muhbianco.com.br` responde com tenant `muhbianco` resolvido por Host; host desconhecido → 404; `alembic upgrade head` + `downgrade -1` verdes em MariaDB no CI; suíte de vazamento roda (mesmo que com poucas rotas); outbox entrega evento de teste a um consumidor com `processed_events`; deploy via pipeline atual atualizado (skill de deploy com mapa `commerce`); Chatwoot em prod construído a partir de `muchatwoot@mb/main` com a mesma imagem funcional.

**Dependências**: acesso root MariaDB para bootstrap; alteração da stack do Traefik (janela curta, reinício do Traefik ~5 s); DNS `staging.*`, `painel.`, `api-commerce.` e `edge.muhbianco.com.br` (A record).
...
**Riscos/rollback**: Let's Encrypt rate limit em testes → usar staging LE em `commerce-staging` (`--certificatesresolvers.lestaging`) ou limitar tentativas; Chatwoot webhook perdido → reconciliação; rollback = desativar flag `storefront` do tenant (storefront volta a 404/“em breve”).
